...
void snap_directory(char *dir_path, int snapfd, char *snap,int offset)
{
    DIR *dir;
    if ((dir = opendir(dir_path)) == NULL)
    {
    ...
    char path[300]; // cale relativa catre fisier(director sau file)
    int datafd;
    if ((datafd = open(snap, O_RDONLY)) < 0)
        {
    ...
    while ((file = readdir(dir)) != NULL)
    {
        if (strcmp(file->d_name, ".") == 0 || strcmp(file->d_name, "..") == 0)
            continue;
        sprintf(path, "%s/%s", dir_path, file->d_name);
        nume_fisier[100] = ""; innode_fisier[100] = ""; dimensiune_fisier[100] = ""; data_fisier[100] = "";
        ...
        mode_t type = in_nod.st_mode;
        if (S_ISDIR(type))
            snap_directory(path,snapfd,snap,offset);
    }
    if (closedir(dir))
    {
        perror(NULL);
...
int main(int argc, char **argv)
{
    ...
    snap_directory(argv[1],snapfd,snap,0);
    // int fd_myfile;
